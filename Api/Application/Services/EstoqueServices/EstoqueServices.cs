...
public class EstoqueServices {
    ...
    public async Task BaixaEstoqueProduto(Pedido pedido) {
        ...
        {
            var pedidoProduto = await _context.Produtos.Include(p => p.EstoqueProduto).FirstOrDefaultAsync(p =>
                    p.Id == item.ProdutoId &&
                    p.LojaId == pedido.LojaId);

            if(pedidoProduto == null)
            {
                throw new BusinessLogicException($"Produto {item.ProdutoId} não encontrado.");
            }
            if(!pedidoProduto.ControlaEstoque)continue;
